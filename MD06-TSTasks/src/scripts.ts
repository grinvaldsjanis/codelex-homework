...
function filterCourses(products: { type: string }[]) {
    return products.filter(product => product.type === 'Course')
}

function getTitles(products: { title: string }[]) {
...
//--- The 'product' is an element of an array 'products'
...
type Product1 = { name: string, kind: string, age: number, title?: string, price?: number };

const anyProduct: Product1 = { name: 'Mr. Fluff', kind: 'British Shorthair', age: 4 }
